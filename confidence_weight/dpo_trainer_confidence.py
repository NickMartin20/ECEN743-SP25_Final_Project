...
class DPOTrainerWithConfidence(Trainer):
    """
    ...
    def get_batch_loss_metrics(
        ...
        if self.sft_model is not None:
            print("SFT model loaded, applying confidence reweighting")
        ...
        losses, chosen_rewards, rejected_rewards = self.dpo_loss(
            model_output["chosen_logps"], model_output["rejected_logps"], ref_chosen_logps, ref_rejected_logps
                ...
                # === Apply SFT confidence weighting if enabled ===
        if self.sft_model is not None:
            with torch.no_grad():
                # Compute log-likelihood of chosen and rejected with SFT model
                chosen_inputs = batch["chosen_input_ids"]
                rejected_inputs = batch["rejected_input_ids"]
                chosen_mask = batch["chosen_attention_mask"]
                rejected_mask = batch["rejected_attention_mask"]

                sft_chosen_logits = self.sft_model(input_ids=chosen_inputs, attention_mask=chosen_mask).logits
                sft_rejected_logits = self.sft_model(input_ids=rejected_inputs, attention_mask=rejected_mask).logits

                log_softmax = torch.nn.functional.log_softmax
                sft_chosen_log_probs = log_softmax(sft_chosen_logits, dim=-1)
                sft_rejected_log_probs = log_softmax(sft_rejected_logits, dim=-1)

                # Get per-token likelihoods
                chosen_ll = sft_chosen_log_probs.gather(-1, chosen_inputs.unsqueeze(-1)).squeeze(-1)
                rejected_ll = sft_rejected_log_probs.gather(-1, rejected_inputs.unsqueeze(-1)).squeeze(-1)

                # Mask padding tokens and average
                chosen_ll = (chosen_ll * chosen_mask).sum(dim=1) / chosen_mask.sum(dim=1)
                rejected_ll = (rejected_ll * rejected_mask).sum(dim=1) / rejected_mask.sum(dim=1)

                confidence = torch.abs(chosen_ll - rejected_ll)
