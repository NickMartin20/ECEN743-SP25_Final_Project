...
def main():
    ...
    # Load datasets
    ###############
    dataset_base_path = os.path.abspath(list(data_args.dataset_mixer.keys())[0])

    if not os.path.exists(dataset_base_path):
        raise FileNotFoundError(f"Dataset path not found: {dataset_base_path}")
  
    raw_datasets = {
      "train": load_from_disk(os.path.join(dataset_base_path, "train_prefs")),
      "test": load_from_disk(os.path.join(dataset_base_path, "test_prefs")),
    }
    ...
    # Skipping decontamination since we're running offline
    logger.info("Skipping HumanEval decontamination step (offline mode).")
    ...
    if is_adapter_model(model_args.model_name_or_path, model_args.model_revision):
        ...
        print(f"PEFT base model path: {peft_config.base_model_name_or_path}")
    ...
    if model_args.use_peft:
        logger.info("Running in PEFT adapter mode (only fine-tuning LoRA weights).")
    else:
        logger.info("Running in full-model fine-tuning mode.")
    #########################
    ...
    trainer = DPOTrainerWithConfidence(
        ...
        peft_config=get_peft_config(model_args),  # <-- this one is OK to pass
      )
